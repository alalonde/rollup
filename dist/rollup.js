/*
	Rollup.js v0.25.4
	Wed Mar 02 2016 23:25:20 GMT-0700 (MST) - commit 22adf5d97757cf060657029b66dfc4425fbf45b8
...
pp$3.parseTopLevel = function (node) {
  var first = true;
...
pp$3.parseStatement = function (declaration, topLevel) {
...
pp$3.parseBreakContinueStatement = function (node, keyword) {
...
pp$3.parseDebuggerStatement = function (node) {
  this.next();
  this.semicolon();
  return this.finishNode(node, "DebuggerStatement");
};

pp$3.parseDoStatement = function (node) {
  this.next();
...
pp$3.parseForStatement = function (node) {
  this.next();
  this.labels.push(loopLabel);
...
pp$3.parseFunctionStatement = function (node) {
  this.next();
  return this.parseFunction(node, true);
};

pp$3.parseIfStatement = function (node) {
  this.next();
  node.test = this.parseParenExpression();
...
pp$3.parseReturnStatement = function (node) {
...
pp$3.parseSwitchStatement = function (node) {
  this.next();
...
pp$3.parseThrowStatement = function (node) {
  this.next();
...
pp$3.parseTryStatement = function (node) {
  this.next();
...
pp$3.parseVarStatement = function (node, kind) {
  this.next();
...
pp$3.parseWhileStatement = function (node) {
  this.next();
  node.test = this.parseParenExpression();
  this.labels.push(loopLabel);
  node.body = this.parseStatement(false);
...
pp$3.parseWithStatement = function (node) {
...
pp$3.parseEmptyStatement = function (node) {
  this.next();
  return this.finishNode(node, "EmptyStatement");
};

pp$3.parseLabeledStatement = function (node, maybeName, expr) {
  for (var i = 0; i < this.labels.length; ++i) {
...
pp$3.parseExpressionStatement = function (node, expr) {
...
pp$3.parseBlock = function (allowStrict) {
...
pp$3.parseFor = function (node, init) {
  node.init = init;
  this.expect(tt.semi);
...
pp$3.parseForIn = function (node, init) {
...
pp$3.parseVar = function (node, isFor, kind) {
  node.declarations = [];
...
pp$3.parseFunction = function (node, isStatement, allowExpressionBody) {
...
pp$3.parseFunctionParams = function (node) {
  this.expect(tt.parenL);
...
pp$3.parseClass = function (node, isStatement) {
  this.next();
...
pp$3.parseClassMethod = function (classBody, method, isGenerator) {
  method.value = this.parseMethod(isGenerator);
  classBody.body.push(this.finishNode(method, "MethodDefinition"));
};

pp$3.parseClassId = function (node, isStatement) {
  node.id = this.type === tt.name ? this.parseIdent() : isStatement ? this.unexpected() : null;
};

pp$3.parseClassSuper = function (node) {
  node.superClass = this.eat(tt._extends) ? this.parseExprSubscripts() : null;
};

// Parses module export declaration.

pp$3.parseExport = function (node) {
  this.next();
  // export * from '...'
  if (this.eat(tt.star)) {
...
pp$3.shouldParseExportStatement = function () {
  return this.type.keyword;
};

// Parses a comma-separated list of module exports.

pp$3.parseExportSpecifiers = function () {
  var nodes = [],
      first = true;
  // export { x, y as z } [from '...']
  this.expect(tt.braceL);
...
pp$3.parseImport = function (node) {
  this.next();
  // import '...'
...
pp$3.parseImportSpecifiers = function () {
  var nodes = [],
      first = true;
...
pp$1.toAssignable = function (node, isBinding) {
...
pp$1.toAssignableList = function (exprList, isBinding) {
...
pp$1.parseSpread = function (refDestructuringErrors) {
  var node = this.startNode();
  this.next();
...
pp$1.parseRest = function (allowNonIdent) {
  var node = this.startNode();
  this.next();
...
pp$1.parseBindingAtom = function () {
...
pp$1.parseBindingList = function (close, allowEmpty, allowTrailingComma, allowNonIdent) {
  var elts = [],
      first = true;
...
pp$1.parseBindingListItem = function (param) {
  return param;
};

// Parses assignment pattern around given atom if possible.

pp$1.parseMaybeDefault = function (startPos, startLoc, left) {
...
pp$1.checkLVal = function (expr, isBinding, checkClashes) {
  switch (expr.type) {
    case "Identifier":
...
pp$2.checkPropClash = function (prop, propHash) {
...
pp$2.parseExpression = function (noIn, refDestructuringErrors) {
...
pp$2.parseMaybeAssign = function (noIn, refDestructuringErrors, afterLeftParse) {
...
pp$2.parseMaybeConditional = function (noIn, refDestructuringErrors) {
  var startPos = this.start,
      startLoc = this.startLoc;
...
pp$2.parseExprOps = function (noIn, refDestructuringErrors) {
  var startPos = this.start,
      startLoc = this.startLoc;
...
pp$2.parseExprOp = function (left, leftStartPos, leftStartLoc, minPrec, noIn) {
...
pp$2.parseMaybeUnary = function (refDestructuringErrors) {
  if (this.type.prefix) {
    var node = this.startNode(),
...
pp$2.parseExprSubscripts = function (refDestructuringErrors) {
  var startPos = this.start,
      startLoc = this.startLoc;
...
pp$2.parseSubscripts = function (base, startPos, startLoc, noCalls) {
  for (;;) {
    if (this.eat(tt.dot)) {
      var node = this.startNodeAt(startPos, startLoc);
      node.object = base;
...
pp$2.parseExprAtom = function (refDestructuringErrors) {
  var node = undefined,
...
pp$2.parseParenAndDistinguishExpression = function (canBeArrow) {
  var startPos = this.start,
      startLoc = this.startLoc,
      val = undefined;
  if (this.options.ecmaVersion >= 6) {
...
pp$2.parseParenArrowList = function (startPos, startLoc, exprList) {
...
pp$2.parseTemplateElement = function () {
...
pp$2.parsePropertyValue = function (prop, isPattern, isGenerator, startPos, startLoc, refDestructuringErrors) {
...
pp$2.parseMethod = function (isGenerator) {
  var node = this.startNode();
  this.initFunction(node);
  this.expect(tt.parenL);
...
pp$2.parseArrowExpression = function (node, params) {
  this.initFunction(node);
...
pp$2.parseFunctionBody = function (node, isArrowFunction) {
...
pp$2.checkParams = function (node) {
  var nameHash = {};
...
pp$2.parseIdent = function (liberal) {
  var node = this.startNode();
...
pp$2.parseComprehension = function (node, isGenerator) {
  node.blocks = [];
